(function() {
	...
	var tabLists = {
		...
		exists: function(id) {
			if (this._lists[id] || id == -1) return true;
			else return false;
		},
		add: function(list) {
		...
		get: function(id) {
			if (id == -1) return this._alltasks;
			else return this._lists[id];
		},
		getAll: function() {
			var r = [];
	...
	var mytinytodo = window.mytinytodo = _mtt = {
		...
		init: function(options) {
			...
			$('.mtt-tabs-select-button').click(function(event) {
				...
				if (!_mtt.menus.selectlist) _mtt.menus.selectlist = new mttMenu('slmenucontainer', {
					onclick: slmenuSelect
				});
	...
	function submitNewTask(form) {
		...
		_mtt.db.request('newTask', {
			...
			$('#taskrow_' + item.id).effect("highlight", {
				color: _mtt.theme.newTaskFlashColor
			}, 2000);
			refreshTaskCnt();
		});
	...
	function setTaskPrio(id, prio) {
		...
		$t.effect("highlight", {
			color: _mtt.theme.editTaskFlashColor
		}, 'normal');
	};
	...
	function setSort(v, init) {
		...
		if (!init) {
			changeTaskOrder();
			if (!flag.readOnly) _mtt.db.request('setSort', {
				list: curList.id,
				sort: curList.sort
			});
		}
	...
	function tabSelect(elementOrId) {
		...
		if (curList.hidden) {
			...
			if (curList.id > 0) _mtt.db.request('setHideList', {
				list: curList.id,
				hide: 0
			});
		}
	...
	function listMenu(el) {
		if (!mytinytodo.menus.listMenu) mytinytodo.menus.listMenu = new mttMenu('listmenucontainer', {
	...
	function saveTaskNote(id) {
		_mtt.db.request('editNote', {
			id: id,
			note: $('#notetext' + id).val()
		}, function(json) {
			if (!parseInt(json.total)) return;
			var item = json.list[0];
	...
	function listOrderChanged(event, ui) {
		...
		_mtt.db.request('changeListOrder', {
			order: order
		});
		_mtt.doAction('listOrderChanged', {
			order: order
		});
	};
	...
	*/
	...
	function flashError(str, details) {
		...
		$("#loading").hide();
		$("#msg").addClass('mtt-error').effect("highlight", {
			color: _mtt.theme.msgFlashColor
		}, 700);
	}
	...
	function flashInfo(str, details) {
		...
		$("#msg>.msg-details").text(details);
		$("#loading").hide();
		$("#msg").addClass('mtt-info').effect("highlight", {
			color: _mtt.theme.msgFlashColor
		}, 700);
	}
	...
	*/
	...
	function logout() {
		$.post(mytinytodo.mttUrl + 'ajax.php?logout', {
			logout: 1
		}, function(json) {
			flag.isLogged = false;
			window.location.reload();
